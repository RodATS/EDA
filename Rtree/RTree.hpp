...
//--------------------------------------------------------
  ...
  void del(const Rectangle<N> &box, const ElemType &value);
//--------------------------------------------------------
  ...
  shared_ptr<Node> find_leaf(const std::shared_ptr<Node> &current_node,
                                    const Rectangle<N> &box,
                                    const ElemType &value);

  shared_ptr<Node> condense_tree(const std::shared_ptr<Node> &current_node,
                                      const Rectangle<N> &box,
                                      const ElemType &value);
...
//delete
template <size_t N, typename ElemType, size_t M, size_t m>
void RTree<N, ElemType, M, m>::del(const Rectangle<N> &box,
                                      const ElemType &value) {
  std::shared_ptr<Node> splitted_node = choose_leaf(root_pointer_, box, value);
  if (!splitted_node) {
    return;
...
shared_ptr<Node> find_leaf(const std::shared_ptr<Node> &current_node,
  ...
  if(!current_node->is_leaf()){
    SpatialObject *entry;
    shared_ptr<Node> next_node = choose_node(current_node, box, entry);
    hared_ptr<Node> splitted_node = find_leaf(next_node, box, value);
  }
  else{
